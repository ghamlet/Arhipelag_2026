...
Press 'S' to save a screenshot to dataset/cubes/digit_{CUBE_NUMBER}/.
...
CUBE_NUMBER = "9"    # - цифра i на модели куба, будет сохраняться в соответствующую папку digit_i
...
SAVE_DIR = os.path.join(DATASET_DIR, "cubes", f"digit_{CUBE_NUMBER}")
...
while True:
    ...
    if key == ord('s') or key == ord('ы'):
        filename = f"cube_{img_counter:04d}.jpg"
