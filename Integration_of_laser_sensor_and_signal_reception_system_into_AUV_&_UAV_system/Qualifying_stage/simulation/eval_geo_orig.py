    ...
    while True:
        try:
            camera_frame = cv2.imdecode(
                np.frombuffer(drone.get_raw_video_frame(), dtype=np.uint8),
                cv2.IMREAD_COLOR
            )
            ...
            if np.all(ids is not None):
                ...
                cv2.aruco.drawDetectedMarkers(camera_frame, corners)
                cv2.aruco.drawAxis(
                    camera_frame, camera_mtx, camera_dist,
                    r_vec_rodrigues, t_vec, 0.01
                )
    ...
    while True:
        if new_point and new_message:
            drone.go_to_local_point(
                x=command_x, y=command_y, z=command_z, yaw=command_yaw
            )
            new_point = False
            new_message = False
        ...
        if p_r and t_vec is not None and r_vec_euler is not None:
            ...
            command_x += k_p_xy * (
                x_camera * c_yaw_prev - z_camera * s_yaw_prev + distance * s_yaw_comm
            )
            command_y += k_p_xy * (
                x_camera * s_yaw_prev + z_camera * c_yaw_prev - distance * c_yaw_comm
            )
...
if __name__ == '__main__':
    ...
    try:
        calibration_path = os.path.join(
            os.path.dirname(__file__), '..', '..', 'optimized_camera_params.yml'
        )
        mtx, dist = load_coefficients(calibration_path)
    except (FileNotFoundError, cv2.error):
        print(
            'Сan not find calibration data, please run get_camera_samples.py script '
    ...
    pos_and_orient = mp.Process(
        target=image_proc, args=(buffer, pioneer_mini, mtx, dist)
    )
    drone_flight = mp.Process(
        target=drone_control, args=(buffer, pioneer_mini)
    )
