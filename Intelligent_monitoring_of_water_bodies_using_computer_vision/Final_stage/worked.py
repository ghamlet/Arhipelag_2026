# мысли вслух. этот код позволяет запускать детекцию на самом дроне с помощью локальной нейроник. класс камеры дает лишь возможность аппаратно подключиться к камере. чтобы показывать видеопоток с камеры нужно эти кадры кидать в Imageviewerи через ртсипи будет досутпно видео. 
